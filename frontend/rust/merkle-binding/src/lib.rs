...
pub fn wasm_sends() -> Result<JsValue, JsValue> {
    let dummy_data = vec![
        Airdrop {
            address: String::from("osmo1mkply7ymvvdk56aamsdftqux2pvws9e6nqfchy"),
            staked: String::from("69"),
        },
        Airdrop {
            address: String::from("osmo1wr3jreg7vrmwlp7ylccshgrv2n53grdfv33hqe"),
            staked: String::from("420"),
        },
    ];
...
pub fn wasm_gets(val: JsValue) -> Result<(), JsValue> {
    ...
    for result in new_data {
        ...
        record.staked =
            ((record.staked.parse::<f64>().unwrap() as f64) * multiplier * 5000000.0).to_string();
